...
var recognizedWord = { x: "nothing" };
$(document).ready(function() {
  wordList = ["up", "down", "left", "right", "stop"];
...
function startListening(recognizedWord) {
  ...
  recognizer.listen(({ scores }) => {
    ...
    recognizedWord.x = scores[0].word;
    $("#recognized-word").text(recognizedWord.x);
...
function Player(maze, c, _cellsize, onComplete, sprite = null) {
  ...
  function check2() {
    $("#ecode").text(recognizedWord.x);
    var cell = map[cellCoords.x][cellCoords.y];
    moves++;

    if (cell.w == true && recognizedWord.x == "left") {
      removeSprite(cellCoords);
      cellCoords = {
        x: cellCoords.x - 1,
        y: cellCoords.y
    ...
    if (cell.n == true && recognizedWord.x == "up") {
      removeSprite(cellCoords);
      cellCoords = {
        x: cellCoords.x,
        y: cellCoords.y - 1
    ...
    if (cell.e == true && recognizedWord.x == "right") {
      removeSprite(cellCoords);
      cellCoords = {
        x: cellCoords.x + 1,
        y: cellCoords.y
    ...
    if (cell.s == true && recognizedWord.x == "down") {
      removeSprite(cellCoords);
      cellCoords = {
        x: cellCoords.x,
        y: cellCoords.y + 1
    ...
    recognizedWord.x = "nothing";

  }
  my_interval = window.setInterval(check2, 0);

  //window.setInterval(check2, 0);
...
var button_stop = document.getElementById("mazeContainer");
